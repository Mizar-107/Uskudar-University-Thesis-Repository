...
\author{
\begin{tabular}{c}
Recep Ertu\u{g}rul Ek\c{s}i\\
\small Department of Computer Engineering\\
\small \"Usk\"udar University\\
\small Istanbul, Turkey\\
\small recepertugrul.eksi@st.uskudar.edu.tr
\end{tabular}
\hspace{1cm}
\begin{tabular}{c}
Rowanda D. Ahmed\\
...
\small \"Usk\"udar University\\
\small Istanbul, Turkey\\
\small rowanda.ahmed@uskudar.edu.tr
\end{tabular}
}
...
The Ultralytics team has continued YOLO development with YOLOv5~\cite{jocher2022yolov5} through YOLO11~\cite{ultralytics2024yolo11}, introducing improvements in training strategies, augmentation pipelines, and architectural refinements. Concurrent developments include YOLOv6~\cite{li2022yolov6} optimized for industrial deployment and YOLOv7~\cite{wang2023yolov7} which introduced efficient layer aggregation networks. YOLO11, released in 2024, features C3k2 blocks for efficient feature extraction, C2PSA (Cross Stage Partial with Spatial Attention) modules, and SPPF (Spatial Pyramid Pooling Fast) for multi-scale feature aggregation. The model family includes variants from nano (n) to extra-large (x), with the small (s) variant offering 9.4 million parameters and 21.5 GFLOPs---suitable for edge deployment scenarios common in maritime surveillance.
...
Recent work has applied deep learning to maritime detection. Shao et al.~\cite{shao2018seaships} introduced the SeaShips dataset and demonstrated CNN-based detection for coastal surveillance. The Singapore Maritime Dataset~\cite{prasad2017video} provided benchmark data for near-shore vessel detection. More recent approaches have incorporated attention mechanisms~\cite{chen2020survey} and improved YOLO architectures~\cite{huang2021survey} for enhanced ship detection in various maritime environments. However, these datasets capture different maritime environments than the Bosphorus, with its unique combination of urban shoreline, bridge infrastructure, and distinctive vessel types.
...
\begin{table}[!ht]
\centering
\caption{Dataset Statistics for the bogaz\_v\_1.v3 Bosphorus Maritime Dataset}
\label{tab:dataset}
\begin{tabular}{lc}
\toprule
...
The architecture employs several key components: Conv layers with varying kernel sizes for hierarchical feature extraction building on ResNet principles~\cite{he2016deep}, C3k2 blocks combining cross-stage partial connections with efficient 3$\times$3 convolutions, SPPF (Spatial Pyramid Pooling Fast) for multi-scale feature aggregation, C2PSA modules incorporating spatial attention mechanisms, and a three-scale detection head inspired by Feature Pyramid Networks~\cite{lin2017feature} for objects at different sizes. The pretrained model was initialized with COCO weights, with 493 of 499 transferable items successfully loaded.
...
The authors thank \"Usk\"udar University for institutional support, Roboflow for dataset hosting and annotation tools, Google Colab for GPU compute resources (NVIDIA L4), and Ultralytics for the YOLO implementation. The bogaz\_v\_1 dataset is available under CC BY 4.0 license.
...
G. Jocher \textit{et al.}, ``YOLOv5 by Ultralytics,'' GitHub, 2022. [Online]. Available: https://github.com/ultralytics/yolov5

\bibitem{li2022yolov6}
C. Li \textit{et al.}, ``YOLOv6: A Single-Stage Object Detection Framework for Industrial Applications,'' \textit{arXiv preprint arXiv:2209.02976}, 2022.

\bibitem{wang2023yolov7}
C.-Y. Wang, A. Bochkovskiy, and H.-Y. M. Liao, ``YOLOv7: Trainable Bag-of-Freebies Sets New State-of-the-Art for Real-Time Object Detectors,'' in \textit{Proc. IEEE Conf. Comput. Vis. Pattern Recognit. (CVPR)}, 2023, pp. 7464--7475.
...
L. Chen, W. Shi, and D. Deng, ``Improved YOLOv3 Based on Attention Mechanism for Fast and Accurate Ship Detection in Optical Remote Sensing Images,'' \textit{Remote Sensing}, vol. 13, no. 4, p. 660, 2021.

\bibitem{huang2021survey}
H. Huang, D. Sun, R. Wang, C. Zhu, and B. Liu, ``Ship Target Detection Based on Improved YOLO Network,'' \textit{Mathematical Problems in Engineering}, vol. 2020, Article ID 6402149, 2020.
...
Bosphorus Vision Project, ``bogaz\_v\_1 Dataset,'' Roboflow Universe, 2024. [Online]. Available: https://universe.roboflow.com/bosphorus-vision-project/bogaz\_v\_1-o9ldr
