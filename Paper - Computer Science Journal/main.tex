...
\title{Fine-tuned YOLO11 for Maritime Vessel Detection in the Bosphorus Strait: A Comparative Study}

\author[Üsküdar University, Istanbul, Turkey, recepertugrul.eksi@st.uskudar.edu.tr]{Recep Ertuğrul Ekşi}

\author[Üsküdar University, Istanbul, Turkey, rowanda.ahmed@uskudar.edu.tr]{Rowanda D. Ahmed}
...
The remainder of this paper is organized as follows: Section~2 reviews related work in YOLO development and maritime object detection. Section~3 details our methodology, including dataset characteristics, model architecture, and training configuration. Section~4 presents experimental results and analysis. Section~5 discusses findings and limitations, and Section~6 concludes with future research directions.
...
We selected YOLO11s (small variant) as our base architecture, balancing model capacity with inference efficiency. Table~\ref{tab:architecture} details the model specifications.
...
Learning rate & 0.001 $\rightarrow$ 0.01 & Cosine annealing \\
...
The input resolution of 1088$\times$1088 was selected to optimize detection of small and distant vessels while maintaining efficient inference. This resolution represents 2.89$\times$ more pixels than the standard 640$\times$640 YOLO input, improving small object detection at the cost of increased computation.
...
\textbf{Case Study---Partial Detections:} The fine-tuned model's 71 false positives often represent partial vessel detections (detecting bow and stern separately) or reflections on water. In safety-critical applications, such over-detection is preferable to missed vessels.
...
\begin{itemize}
    ...
    \item Incurs only modest speed penalty (20--30\% slower) due to efficient YOLO architecture
...
Future work will extend this research in several directions: multi-class vessel classification (cargo, tanker, ferry, fishing), night-time and thermal imaging adaptation, video-based tracking with temporal consistency, edge deployment optimization for embedded systems, and evaluation of generalization to other maritime corridors. Integration with AIS data for vessel identification verification presents an additional avenue for comprehensive maritime surveillance systems.

\begin{acknowledgements}
The authors thank Üsküdar University for institutional support, Roboflow for dataset hosting and annotation tools, Google Colab for GPU compute resources (NVIDIA L4), and Ultralytics for the YOLO implementation. The bogaz\_v\_1 dataset is available under CC BY 4.0 license at \url{https://universe.roboflow.com/bosphorus-vision-project/bogaz_v_1-o9ldr}.
